...
function renderAmiibo($amiibo)
{
    ...
    $replace = [$amiibo->getTitle(), $amiibo->getSubtitle(), $amiibo->getDescription(), $amiibo->getPrice(), $amiibo->getImage()];
...
function renderTshirt($tshirt)
{
    ...
    $replace = [$tshirt->getTitle(), $tshirt->getDescription(), implode(" - ", $tshirt->getSizes()), $tshirt->getPrice(), $tshirt->getImage()];
    return str_replace($search, $replace, $template);    
}
